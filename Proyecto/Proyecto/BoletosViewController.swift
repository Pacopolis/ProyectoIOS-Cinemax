...
class BoletosViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func confirmar(_ sender: Any) {
        ...
        if requeridos <= disponibles! {
            funcion?.venderBoletos(cantidad: requeridos)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "conditionSegue" {
            ...
            viewController.transaccion = VentaBoletos(funcion: self.funcion!, cantidadAdultos: adultos.selectedRow(inComponent: 0), cantidadNinos: ninos.selectedRow(inComponent: 0))
        }
    }
